    ...
    ($helper_name:ident, $settings_ty:ty, $settings_name:ident, $result:ident) => {{
        #[allow(non_camel_case_types)]
        {
            ...
            simd_runtime_generate!(
                fn gen_noise($settings_name: &$settings_ty, result: &mut [MaybeUninit<f32>]) {
...
pub mod get_1d_noise {
    ...
    #[allow(dead_code)]
    pub fn get_1d_noise(noise_type: &NoiseType, result: &mut [MaybeUninit<f32>]) {
        match noise_type {
            NoiseType::Fbm(s) => {
...
pub mod get_2d_noise {
    ...
    /// in a single pass.
    #[allow(dead_code)]
    pub fn get_2d_noise(noise_type: &NoiseType, result: &mut [MaybeUninit<f32>]) {
        match noise_type {
            NoiseType::Fbm(s) => {
...
pub mod get_3d_noise {
    ...
    /// in a single pass.
    #[allow(dead_code)]
    pub fn get_3d_noise(noise_type: &NoiseType, result: &mut [MaybeUninit<f32>]) {
        match noise_type {
            NoiseType::Fbm(s) => {
...
pub mod get_4d_noise {
    ...
    use crate::{FbmSettings, GradientSettings, RidgeSettings, TurbulenceSettings};
    #[allow(dead_code)]
    pub fn get_4d_noise(noise_type: &NoiseType, result: &mut [MaybeUninit<f32>]) {
        match noise_type {
            NoiseType::Fbm(s) => {
